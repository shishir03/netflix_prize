import netflixprize.fileio.*;

import java.io.IOException;
import java.util.ArrayList;

public class NetflixTester {
	public static final String baseFile = "data" + FileIO.fileSeparator + "testfiles"
			+ FileIO.fileSeparator + "5Ratings.csv";
	public static final String testFile = "data" + FileIO.fileSeparator + "testfiles" 
			+ FileIO.fileSeparator + "5Tests.csv";
	...
	public static final String tagsFile = "data" + FileIO.fileSeparator + "ml-latest-small" 
			+ FileIO.fileSeparator + "tags.csv";
	
	public static void main(String[] args) throws IOException {
		System.out.println("\n***Starting the clock***");
		long startTime = System.currentTimeMillis();
		System.out.println("\n***Initializing Predictor***");
		NetflixPredictor tester = new NetflixPredictor(moviesFile, baseFile, tagsFile, 
				linksFile);
		...
		
		{
			...
			while(numberTested < 20) {
				...
				if (val >= 0) {
					System.out.println
					("Rating returned from getRating for a non-existing user! ");
					System.out.println
					("(This probably means you're reading in the database incorrectly)");
					System.exit(0);
				}
				numberTested++;
			}
			...
			while(numberTested < 20) {
				...
				if (val >= 0) {
					System.out.println("Rating returned from getRating for a movie that "
							+ "was not rated by user!");
					System.out.println("(This probably means you're reading in the "
			...
			while(numberTested < 20) {
				...
				if (val < 0) {
					System.out.println("No rating returned from getRating for a movie that "
							+ "was rated by user!");
					System.out.println("(This probably means you're reading in the database "
							+ "incorrectly)");
					System.exit(0);
				}
				numberTested++;
		...
		for (String line : testDataLines) {
			...
			numberTested++;
			System.out.println("Tested " + (numberTested+numberPresent) + "/" 
			+ testDataLines.size());
		}
		...
		System.out.println("\n\n******Results******");
		System.out.println("Runtime: " + secs + " s");
		System.out.println("Test combinations already present: " + numberPresent);
		...
		totalDifference/numberTested);
		System.out.println("Root mean square difference: "
